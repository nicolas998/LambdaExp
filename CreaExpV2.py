    ...
    for d1, d2 in zip(Dates[:-1], Dates[1:]):
        ...
        for c,rc in enumerate(args.rc):
            ...
            L = [d_initial, d_end, params, args.link, initFlag,
                ...
                fl.Evento.path+'/ForRun/'+name+'_'+lambdaName+'.gbl'
                ]
            #Updates the list 
            Lista.append(L)
            ...
            if c == len(args.rc)-1:
                #Updates exec file 
                fl.Evento.CreateBashFile(name+'_'+lambdaName, status = 'update', last = True)
            else:
                fl.Evento.CreateBashFile(name+'_'+lambdaName, status = 'update')
